...
render_waveform (cairo_surface_t * surface, const RENDER *render, SNDFILE *infile, SF_INFO *info, double left, double top, double width, double height, int channel)
{
	...
	if (!render->rectified) { // center line
		drect pts = {left, top + (height/2.0)-.5, left + width,  top + (height/2.0) +.5};
	...
	while ((sf_read_float(infile, data, buffer_len))>0){
		...
		for(frame=srcidx_start;frame<srcidx_stop;frame++){ 
			...
			for(ch=0;ch<info->channels;ch++){
				if (channel>0 && (ch+1)!=channel) continue;
				if(frame * info->channels + ch > buffer_len){ fprintf(stderr, "index error!\n"); break; }    
...
{	int cnt, i;
	...
	const double dx = rect?distance:(distance/2.0);
	const double dd = rect?0.0:(distance/2.0);
...
str_print_timecode (char * text, int text_len, double sec, int fps_num, int fps_den)
{
	...
	int frame = (int)floor((sec-floor(sec))*(double)fps_num/(double)fps_den);
...
render_timecode (cairo_surface_t * surface, const RENDER * render, double left, double width, double seconds, double top, double height)
{
	...
	{	double yoff = 0;
		y_line (cr, left + ticks.distance [k], top + height, TICK_LEN) ;
		if (k % 2 == 1) yoff=  1.0 * NORMAL_FONT_SIZE;
...
render_to_surface (const RENDER * render, SNDFILE *infile, SF_INFO *info, cairo_surface_t * surface)
{
	...
	if (render->channel < 0) {
		...
		const double mheight=(height - (info->channels-1) * chnsep)/(double)info->channels;
		int ch;
		for(ch=0;ch<info->channels;ch++){
			render_waveform (surface, render, infile, info,
					(render->border?LEFT_BORDER:0),
					(render->border?TOP_BORDER:0) + ((mheight+chnsep)*(double)ch),
					width, mheight, ch+1) ;

			if (render->border)
				render_wav_border (surface, render,
						LEFT_BORDER, width,
						TOP_BORDER + ((mheight+chnsep)*(double)ch), mheight) ;
			else if (ch>0 && chnsep > 0) {
				cairo_rectangle(cr, 0, ((mheight+chnsep)*(double)ch)-chnsep, render->width, chnsep);
				cairo_stroke_preserve(cr);
				cairo_set_source_rgba (cr, C_COLOUR(&render->c_bg));
				cairo_fill(cr);
			}
		...
		render_waveform (surface, render, infile, info, (render->border?LEFT_BORDER:0.0), (render->border?TOP_BORDER:0.0), width, height, render->channel) ;
		if (render->border)
...
render_sndfile (RENDER * render)
{
	...
	render->tc_off /= (double) (info.samplerate);
...
{	RENDER render =
	...
	while ((c = getopt_long (argc, argv, 
			   ...
			   "2"	/* --no-rms */
			   "V",	/* --version */
			   long_options, (int *) 0)) != EOF) switch (c) {
	case 'A':		/* --annotation */
